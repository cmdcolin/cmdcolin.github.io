...
- Fixed issue with some GFF3Tabix tracks having some inconsistent layout of
  ...
  really get a unique ID of a feature based on its file offset. Therefore this
  takes the full crc32 of a line as its unique ID.
...
- Fixed bug where older browsers e.g. IE11 were not being properly supported via
  babel. This was an absolutely terrible bug that I found over thanksgiving
...
- We also had behind the scenes work by igv.js team member jrobinso who helped
  ...
  that a read and its mate pair would consistently be assigned the same read
