...
orientation, so for a piece aligned to the minus strand the clip at the start of
the read is the one at the _end_ of its `CIGAR`. Take the leading clip for `+`
pieces and the trailing clip for `-` pieces (the `SA` tag gives you the strand
of each piece alongside its `CIGAR`), and you have every piece measured on the
same axis: the original read, 5' to 3'. A larger clip means the piece starts
further along that read, so the clip lengths give the order of the pieces —
telling us where each came from in the original read, which we plot in a
synteny-style display. Get the strand wrong and the pieces of an inversion sort
backwards, which is exactly the case you were trying to look at.
