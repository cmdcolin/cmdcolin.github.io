...
primary alignment and each of the pieces of the `SA` tag (a semicolon-separated
list of chunks). It then sorts them by the amount of soft-clipping: the
soft-clipping values progressively trim off more of the `SEQ`, telling you each
piece aligned further and further along in the long read. This tells us where
each piece of the split alignment came from in the original `SEQ`, so we can
plot the alignments of the read vs the reference genome using a synteny-style
display.
