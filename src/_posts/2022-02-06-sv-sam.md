...
understand what is going on with your reads, you really need to play around with
...
(https://jbrowse.org/jb2/docs/user_guides/sv_inspector_view/). The SV inspector
and Breakpoint split view work best on Breakends (e.g. VCF 4.3 section 5.4) and
`<TRA>` (translocation) events from `VCF`, or `BEDPE` formatted SV calls, and
you can launch the "breakpoint split view" from the "SV inspector"

![](/media/breakpoint_split_view.png)

### Visualizing a 'read vs reference' view given a split alignment

Given any one record of a split read, we can construct what that split looks
like compared to the reference genome. We don't actually need the primary
specifically: the record's `SA` tag lists every other piece of the split, so the
record plus its `SA` tag together describe the whole read.

The primary is still special in one respect: it carries the `SEQ` of the entire
read (with soft-clipping marking which part aligned), so if you want the full
read sequence to display the bases, the primary is where it lives. Whether the
supplementary records also carry the full `SEQ` depends on the aligner: by
default `minimap2` and `bwa` hard-clip supplementary alignments so they store
only their own aligned portion, but the `-Y` flag makes them soft-clip and keep
the entire `SEQ`. The reconstruction below does not rely on this either way.

We can then construct how the entire read, not just a particular record of the
split alignment, compares to the genome. In JBrowse 2 we implemented this, and
it uses a synteny-style rendering. [1]
...
In order to do this reconstruction, JBrowse 2 takes the `CIGAR` of the record
you started from and each piece listed in its `SA` tag (a semicolon-separated
list of chunks). It then sorts them by the length of the clip at the start of
the read, counting soft (`S`) and hard (`H`) clips alike — this matters because,
as noted above, the supplementary records may be either soft- or hard-clipped
depending on the aligner, so the reconstruction can't assume one or the other. A
larger leading clip means the piece starts further along in the read, so these
clip lengths give the order of the pieces along the original read. This tells us
where each piece of the split alignment came from in the original read, so we
can plot the alignments of the read vs the reference genome using a
synteny-style display.
