...
wasm code, in `hello-wasm`, and one will be an instance of `create-react-app`
...
Add `"hello-wasm":"^1.0.0"` to the `dependencies` object in `app/package.json`.
...
`index.wasm` file. Then it is up to the consumer's bundler to package that
correctly.
...
e.g. `yarn start` in the `app` folder. Therefore, changes to the rust require
