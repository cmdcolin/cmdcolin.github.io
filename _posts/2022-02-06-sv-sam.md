...
A `SAM` file contains "reads" from a sequencer. The raw unaligned reads are
often stored in `FASTQ` format files. The reads from `FASTQ` format are then
inputted into an aligner such as `bwa`, `bowtie`, `minimap2` (there are many
others) which map the reads to a reference genome, and output `SAM` format.
...
A slightly modified command converts `SAM` to `CRAM` (requires -T argument to
...
in your `SAM` file, marked as unmapped using the flag column. Sometimes,
"dumpster diving" (looking at the unmapped records from a `SAM` file) can be
used to aid structural variant searches (e.g. there may be novel sequence in
there not from the reference genome that could be assembled)
...
Examples of `CIGAR` strings

- A `CIGAR` strings could look be `50M` which means 50 bases "matched". There
...
- Another `CIGAR` string with insertions and deletions could be `50M1D1I50M`.
...
- Another `CIGAR` string with soft-clipping `500S50M` this means that 500 bases
