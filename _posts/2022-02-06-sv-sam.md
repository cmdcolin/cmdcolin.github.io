...
- A `SAM` file is **generally** produced when an aligner takes in raw unaligned reads
  (often stored in `FASTQ` format files) and aligns them to a reference genome [3].

- A `SAM` file is a text format that you can read with your text editor. `BAM` and
  `CRAM` are compressed representations of the `SAM` format.
...
If you are converting `SAM` to `CRAM`, it may require the -T argument to
specify your reference sequence (this is because the `CRAM` is "reference
compressed")
...
created a _de novo_ assembly, you could align the contigs of the _de novo_ assembly
to a reference genome and store the results in `SAM`.
...
[3] Examples of programs that do alignment include `bwa`, `bowtie`, and `minimap2`
(there are many others). These programs all can produce `SAM` outputs
...
Note: if a read failed to align to the reference genome, it may still be in
your `SAM` file, marked as unmapped using the flag column. Sometimes, "dumpster
...
- _Finding mismatches_: A `CIGAR` string match like `50M` means 50 bases
  ...
  extended `CIGAR` that replaces `M` with `=` (exact match) and `X` (mismatch).
  Also see Appendix D on the `MD` tag and finding where the mismatches are, but
  note that `MD` tag is tricky
...
- _Split records and soft-clipping_: A `CIGAR` string with soft-clipping
  ...
  `SAM` file, linked by the `SA` (supplmentary alignment) tag, might contain
...
read may map to `chr1` and part of it maps to `chr4`. It is worth reading the
...
SAM file, and note that all the records also have the same read name, or `QNAME` (first
column of `SAM`).
...
flag. Only the "primary" record generally has a `SEQ` field. The split
alignments are generally also all given a `SA` tag that gives info on where all
the other parts of the split are. See
[SAMtags.pdf](https://samtools.github.io/hts-specs/SAMtags.pdf) for more info
on the `SA` tag.

Note: split alignments are different from "multi-mappers" where the entire read
maps maps equally well to, say, `chr4` and `chr1`. Split reads maps part to
chr1, and part to `chr4`. See again the
...
have an `SA` tag with 3 segments, so 3\*4 segments will be documented in the
`SA` tag. In many cases, this is not a problem, but if you imagine a finished
...
([https://jbrowse.org/jb2/docs/user_guide/#sv-inspector](https://jbrowse.org/jb2/docs/user_guide/#sv-inspector).
The SV inspector and Breakpoint split view work best on Breakends (e.g. VCF 4.3
section 5.4) and `<TRA>` (translocation) events from `VCF`, or `BEDPE`
...
[1] Similar functionality also exists in GenomeRibbon
[https://genomeribbon.org](https://genomeribbon.org)
...
[https://jbrowse.org/jb2/docs/user_guide/#sort-color-and-filter-by-tag](https://jbrowse.org/jb2/docs/user_guide/#sort-color-and-filter-by-tag)
...
colored as red (from
[https://jbrowse.org/docs/paired_reads.html](https://jbrowse.org/docs/paired_reads.html)).
...
[https://software.broadinstitute.org/software/igv/interpreting_pair_orientations](https://software.broadinstitute.org/software/igv/interpreting_pair_orientations)

![](/media/inverted_duplication.png)

Figure: JBrowse 2 showing an inverted (tandem) duplication in 1000 genomes
...
directions e.g. `<--` and `-->`, while blue arrows which can indicate an
inversion point in the same direction e.g. `-->` and `-->`
...
[https://github.com/pysam-developers/pysam/issues/667#issuecomment-381521767](https://github.com/pysam-developers/pysam/issues/667#issuecomment-381521767)
...
[https://gist.github.com/cmdcolin/9f677eca28448d8a7c5d6e9917fc56af](https://gist.github.com/cmdcolin/9f677eca28448d8a7c5d6e9917fc56af) for a short
...
Screenshot showing coverage in `BigWig` format from nanopore reads on normal
...
reference genome (and the `SAM` format is a signature of this: it stores data
...
operations ([https://twitter.com/lh3lh3/status/1362921612690010118/photo/1](https://twitter.com/lh3lh3/status/1362921612690010118/photo/1))
as _de novo_ assembled genomes improve and become more widespread, we may see a
...
format) by storing graph node labels in the `target name` slot of `PAF`, and
can refer to an `rGFA` (reference genome graph)! Looking forward to the graph
genome world.
...
- Aligning _de novo_ assembly vs a reference genome
  ([https://twitter.com/lh3lh3/status/1362921612690010118/photo/1](https://twitter.com/lh3lh3/status/1362921612690010118/photo/1)
  which can output `SAM`, but it can also output
...
So you can interpret e.g. F1R2 in relation to being a paired end read (fr) or mate pair (rf) below and with this link [https://software.broadinstitute.org/software/igv/interpreting_pair_orientations](https://software.broadinstitute.org/software/igv/interpreting_pair_orientations)
...
([http://www.htslib.org/workflow/fastq.html](http://www.htslib.org/workflow/fastq.html))
shows how you can stream directly from `FASTQ` to `CRAM` (and generate the
index file .crai too)
...
([https://vincebuffalo.com/notes/2014/01/17/md-tags-in-bam-files.html](https://vincebuffalo.com/notes/2014/01/17/md-tags-in-bam-files.html)).
You have to combine it with the `CIGAR` to get the position of the mismatches
...
The `MD` tag is also not required to exist, but the command `samtools calmd yourfile.bam --reference reference.fa` can add `MD` tags to your `BAM` file. It
is generally not useful for `CRAM` because `CRAM` actually does store
mismatches with the reference genome in it's compression format. Note that
there are also some oddities about `MD` tag representation leading to
complaints (e.g.
[https://github.com/samtools/hts-specs/issues/505](https://github.com/samtools/hts-specs/issues/505))
...
are on the read and ref position with the `CIGAR` string (a la Appendix B)
