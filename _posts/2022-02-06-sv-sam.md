...
are often stored in `FASTQ` format files. The reads from `FASTQ` format are
then inputted into an aligner such as `bwa`, `bowtie`, `minimap2` (there are
many others) which map the reads to a reference genome, and output `SAM`
format.
